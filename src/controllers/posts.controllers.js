...
export async function timelinePosts(req, res) {
   ...
   try {
      const latestPosts = await connection.query(`SELECT users."userName" AS name, posts."userId",users.picture, posts.id ,posts.description AS text, posts.link, posts.title, posts.preview, posts.pic FROM posts JOIN users on users.id = posts."userId" ORDER BY posts.id DESC LIMIT 20;`);
